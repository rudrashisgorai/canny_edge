...
SC_MODULE(Non_Max_Supp)
{
	...
	*******************************************************************************/
	void non_max_supp(int nrows, int ncols, unsigned char *result)
	{
	    ...
	    short m00,gx,gy;
	    float mag1,mag2,xperp,yperp; // floating-point arithmetic (effective FPU)
	    //int mag1,mag2,xperp,yperp; // fixed-point arithmetic (not effective on RPi4)
      ...
      rowcount<nrows-1; /* bug fix 10/05/23, RD */
         ...
         resultptr=resultrowptr;colcount<ncols-1; /* bug fix 10/05/23, RD */
		    ...
		    // floating-point arithmetic (effective FPU)
	            xperp = -(gx = *gxptr)/((float)m00);
	            yperp = (gy = *gyptr)/((float)m00);
		    // fixed-point arithmetic (not effective on RPi4)
		    //gx = *gxptr;
		    //gy = *gyptr;
		    //xperp = -(gx<<16)/m00;
		    //yperp = (gy<<16)/m00;
	...
	void main(void)
	{
	   ...
	   while(1)
	   {
	      ...
	      wait(154,SC_MS);	// floating-point arithmetic (effective FPU)
	      //wait(488,SC_MS);// fixed-point arithmetic (not effective on RPi4)
